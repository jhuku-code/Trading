...
limit = st.sidebar.number_input("OHLCV limit", value=90, min_value=10, max_value=2000)
...
def compute_returns(price_df):
    ...
    for lb in lookbacks:
        if len(price_df) > lb:
            results[f"{lb}d"] = (
                price_df.iloc[-1] - price_df.iloc[-1-lb]
            ) / price_df.iloc[-1-lb]
...
if st.sidebar.button("🔄 Fetch Data"):
    ...
    # ---------------- Excess ----------------
    excess = numeric_returns - theme_median
    excess.columns = [c + "_Excess" for c in excess.columns]

    # ---------------- Combine Coin Level ----------------
    final = pd.concat([numeric_returns, excess], axis=1)
    final["Theme"] = returns["Theme"]
    final["Coin"] = final.index

    # ---------------- Theme Level Excess ----------------
    global_avg = numeric_returns.mean()
    ...
    final[numeric_cols] = (final[numeric_cols] * 100).round(2)
