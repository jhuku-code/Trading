    ...
    if not excel_path.exists():
        ...
        try:
            ...
            if not ("symbol" in cols_lower and "theme" in cols_lower):
                ...
                # --- expose the price_theme + theme mapping + friendly timeframe to other pages ---
...
if st.session_state.final_df is None or st.session_state.final_df.empty:
    ...
    if not selected_themes:
        ...
        display_avg = display_df.iloc[0:0]
        display_coins = display_df.iloc[0:0]
    else:
        # Filter to selected themes
        filtered = final_df[final_df['Theme'].isin(selected_themes)].copy()

        # Identify averages vs coins
        filtered['is_avg'] = filtered['Coin'].astype(str).str.endswith('_average')
        display_avg = filtered[filtered['is_avg']].copy().reset_index(drop=True)
        display_coins = filtered[~filtered['is_avg']].copy().reset_index(drop=True)

        # maintain the sorting: Theme, averages first, then alphabetical coin
        display_avg = display_avg.sort_values(by=['Theme', 'Coin']).reset_index(drop=True)
        display_coins['avg_rank'] = (~display_coins['Coin'].astype(str).str.endswith('_average')).astype(int)
        display_coins = display_coins.sort_values(by=['Theme', 'Coin']).reset_index(drop=True)

        # final combined counts for user
        display_df = pd.concat([display_avg, display_coins], ignore_index=True)

        # drop helper cols if present
        for df in [display_avg, display_coins, display_df]:
            for col in ['is_avg', 'avg_rank']:
                if col in df.columns:
                    df.drop(columns=[col], inplace=True, errors='ignore')

    # Show counts and dataframes: averages first, then coins
    st.write(f"Showing theme averages: {len(display_avg)} rows; coins: {len(display_coins)} rows. Total: {len(display_df)} rows for {len(selected_themes)} theme(s).")

    if len(display_avg) > 0:
        with st.expander(f"Theme Averages ({len(display_avg)} rows) — shown first", expanded=True):
            st.dataframe(display_avg)

    if len(display_coins) > 0:
        with st.expander(f"Coins ({len(display_coins)} rows)", expanded=True):
            st.dataframe(display_coins)
