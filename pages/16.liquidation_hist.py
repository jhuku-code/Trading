...
def compute_all(run_clear_cache: bool = True):
    ...
    try:
        ...
        st.session_state["total_long_series"] = total_long
        st.session_state["total_short_series"] = total_short
        ...
        # --- NEW: long-to-short raw ratio (per-symbol) ---
        # handle division by zero gracefully
        raw_ratio = liq_long_data.div(liq_short_data.replace(0, np.nan))
        raw_ratio = raw_ratio.replace([np.inf, -np.inf], np.nan)

        latest_raw_ratio = raw_ratio.iloc[-1]
        latest_raw_ratio_clean = latest_raw_ratio.dropna()

        sorted_raw_ratio_desc = latest_raw_ratio_clean.sort_values(ascending=False)
        sorted_raw_ratio_asc = latest_raw_ratio_clean.sort_values(ascending=True)
...
# ---------------------------------------------------------
# Charts: Totals and Per-symbol time series
# ---------------------------------------------------------
...
with col2:
    ...
    if selected_symbol_ratio:
        df_ratio_sym = pd.DataFrame({
            f"{selected_symbol_ratio} - Long/Short Ratio": raw_ratio[selected_symbol_ratio]
        })
        st.line_chart(df_ratio_sym)

# ---------------------------------------------------------
# Existing Top/Bottom Z-score & Share tables (unchanged but reorganized)
# ---------------------------------------------------------
