...
def fetch_liquidations(symbols, months_back: int, interval: str, api_key: str, convert_usd_flag: bool):
    ...
    # Ensure numeric and consistent datetime index
...
def compute_all(run_clear_cache: bool = True):
    try:
        ...
        # Coerce numeric (already done in fetch but extra safety)
        ...
        # Z-scores (drop columns that are all NaN to avoid division 0)
        ...
        # latest per-symbol z-scores (dropna)
        ...
        # latest snapshots (dropna where necessary)
        ...
        # Z-scores sorted (dropna before sorting)
        ...
        # raw long/short ratio time series per symbol
        ...
        # store computed objects in session_state
        ...
        st.session_state["raw_ratio"] = raw_ratio
        st.session_state["latest_raw_ratio"] = latest_raw_ratio
        st.session_state["sorted_raw_ratio_desc"] = sorted_raw_ratio_desc
        st.session_state["sorted_raw_ratio_asc"] = sorted_raw_ratio_asc
...
# ---------------------------------------------------------
# Charts: Totals and Per-symbol time series (all line charts)
# ---------------------------------------------------------
...
with colA:
    ...
    df_totals = df_totals.dropna(how="all")  # drop rows where both are NaN
    if df_totals.empty:
        st.info("No total liquidation time series available to plot.")
    else:
...
# ---------------------------------------------------------
# Long-to-Short Ratio: Top/Bottom & Symbol time series
# ---------------------------------------------------------
...
with col2:
    ...
    selected_symbol_ratio = st.selectbox("Symbol for ratio time series (searchable)", options=symbols_list, key="ratio_sym")

    if selected_symbol_ratio:
        if selected_symbol_ratio not in raw_ratio.columns:
            st.info(f"No ratio time series for {selected_symbol_ratio}.")
        else:
            df_ratio_sym = pd.DataFrame({f"{selected_symbol_ratio} - Long/Short Ratio": raw_ratio[selected_symbol_ratio]})
            df_ratio_sym = df_ratio_sym.dropna()
            if df_ratio_sym.empty:
                st.info(f"No historical long/short ratio values for {selected_symbol_ratio}.")
            else:
                st.line_chart(df_ratio_sym)

# ---------------------------------------------------------
# Existing Top/Bottom Z-score & Share tables (fixed formatting)
# ---------------------------------------------------------
