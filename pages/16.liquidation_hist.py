I have the below code for performance for long and short liquidation ratios. I want to understand the code and add create time series chart of total (sum) long and short liquidations across symbols. also add time series chart of long and short liquidations for a specific symbol (which can be selected from a dropdown with autofill option). 
Also calculate long to short liquidation ratio (based on raw values) for all symbols and create tables which show the top/bottom 15 symbols based on the latest value of ratio. Additionally, create a time series chart of the ratio for a specific symbol (which can be selected from a dropdown with autofill option).
