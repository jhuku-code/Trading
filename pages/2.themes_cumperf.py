...
with col_right:
    st.write("Use the Period controls below to select the displayed window. The selected window is rebased to 100.")

if price_theme is None or (isinstance(price_theme, pd.DataFrame) and price_theme.empty):
    st.info("No `price_theme` found.")
    st.stop()

if theme_values is None:
    st.warning("No `theme_values` mapping found.")
    st.stop()

tf_label = price_timeframe if price_timeframe is not None else "unknown"
period_desc = {
    "daily": "1 period = 1 day",
    "4hourly": "1 period = 4 hours",
    "1hourly": "1 period = 1 hour"
}.get(tf_label, f"1 period = data timeframe ({tf_label})")
...
def slice_and_rebase_by_periods(cum_df, view):
    ...
    df_slice = cum_df.tail(n)
    rebased = df_slice.divide(df_slice.iloc[0]).mul(100)
    return rebased, len(cum_df) - n, n
